...
`lib/email.js` arma el HTML de los tres tipos de email (resumen, alerta, bienvenida) con [MJML](https://mjml.io) en vez de tablas escritas a mano: los componentes (`mj-section`, `mj-column`, `mj-wrapper`, `mj-button`, etc.) se compilan a `mjml2html()` recién al momento de enviar, generando el meta viewport y los media queries necesarios para que las columnas se apilen en mobile en vez de achicarse — antes esto se mantenía a mano y era la fuente de los problemas de responsividad/estilos. Limitación a tener en cuenta: `mj-wrapper` no puede anidarse dentro de otro `mj-wrapper` (por eso cada "tarjeta" de ruta/alerta es su propio `mj-wrapper` al tope del email, no anidado dentro de un sobre único). El contenido de texto/badges/íconos sigue siendo HTML crudo embebido en `mj-text` (MJML no obliga a reescribir eso).
