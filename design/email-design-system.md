...
Estructura ya implementada en `buildRouteCards`/`buildAlertCardHtml` (`email.js:319-411,
658-731`), con los ajustes de este documento:

1. Borde `color.border` 1px + borde superior 4px del color de la aerolínea. Radio `radius.card`
   (12px, no 8px).
2. Header: ruta en bold (`cardRouteTitle`, 18px — sin pin, ver §5) a la izquierda; badge de
   aerolínea (código IATA en pill de color, `buildAirlineBadgeHtml`) a la derecha.
3. Tag de tipo de viaje (`buildTripTypeTag`) — pill `color.info`, sin cambios.
4. Tramo(s): salida/llegada con conector (ícono avión + duración/escalas), separador punteado
   entre ida y vuelta si es roundtrip. Sin cambios — ya es el patrón correcto.
5. Badge de hito (mínimo histórico / delta de precio) — ver reemplazo de `color.accent` en §5.
6. Footer: precio grande (28px bold) + conversión ARS/BRL chica debajo, a la izquierda; botón
   CTA a la derecha (`radius.button`, `color.primary`).
