...
function buildDashboardRouteBoxMjml(route) {
  ...
  // Border/background/radius live on the inner box (not on the mj-column's
  // own cell) so the column's own padding is pure gap, not inset-inside-the-
  // border content padding -- otherwise two stacked columns (which is what
  // happens to this same-row pair on a narrow screen) end up with zero space
  // between them, since MJML gives columns no margin of their own.
  const boxStyle = `background:${COLOR_SURFACE};border:1px solid ${COLOR_BORDER};border-radius:${RADIUS_CARD};padding:12px;display:block;`;
  const content = route.link
    ? `<a href="${route.link}" style="text-decoration:none;color:inherit;${boxStyle}">${inner}</a>`
    : `<div style="${boxStyle}">${inner}</div>`;
  return `<mj-column width="50%" padding="8px"><mj-text padding="0">${content}</mj-text></mj-column>`;
}
...
// Each column carries its own gap (see buildDashboardRouteBoxMjml) so boxes
// stay separated whether they stack (mobile) or sit side by side (desktop).
function buildDashboardBoxesMjml(routeList) {
  ...
  for (let i = 0; i < routeList.length; i += 2) {
    ...
    rows.push(mjSection(buildDashboardRouteBoxMjml(pair[0]) + secondColumn));
  }
  return rows.join("");
}
...
const PERSON_NAMES = { pablo: "Pablo", david: "David" };
...
const RECIPIENT_ENV = { pablo: "PABLO_EMAIL", david: "DAVID_EMAIL" };
