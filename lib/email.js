...
// --- MJML layout helpers ------------------------------------------------
// MJML compiles mj-section/mj-column into the responsive table markup email
// clients need (with per-client media queries + a real viewport meta tag),
// so columns actually stack on narrow screens instead of squeezing --
// unlike the hand-written fixed-width <table> layout this replaces. These
// three helpers cover every row shape used below (single/two/three columns);
// padding defaults to 0 so spacing stays explicit and predictable, matching
// how the original table cells had no implicit padding either.
...
function section3(a, b, c, wa, wb, wc, attrs = "") {
  return mjSection(mjColumn(wa, a) + mjColumn(wb, b) + mjColumn(wc, c), attrs);
}
...
function airportInnerHtml(label, primary, secondary) {
  return `<div style="font-size:14px;font-weight:600;color:${COLOR_TEXT_SECONDARY};text-transform:uppercase;letter-spacing:0.5px;">${label}</div>
...
function connectorInnerHtml(centerLabel) {
  return `${centerLabel ? `<div style="font-size:12px;color:${COLOR_TEXT_SECONDARY};margin-bottom:4px;">${centerLabel}</div>` : ""}
...
// date instead of dropping the destination side entirely. Returns MJML
// fragments (a label row + a 3-column row), meant to be nested inside a
// card's <mj-wrapper> (see buildTripCardMjml).
function buildLegRowMjml(legLabel, dateLabel, legSchedule, origin, destination) {
  const labelSection = legLabel
    ? section1(
        `<mj-text padding="0"><div style="font-size:14px;font-weight:600;color:${COLOR_TEXT_SECONDARY};text-transform:uppercase;letter-spacing:0.5px;">${legLabel}</div></mj-text>`,
        `padding-bottom="8px"`
      )
    : "";
  ...
  if (!legSchedule || !legSchedule.segments?.length) {
    const row = section3(
      `<mj-text padding="0" align="left">${airportInnerHtml("Salida", dateLabel, origin.name)}</mj-text>`,
      `<mj-text padding="0" align="center">${connectorInnerHtml("")}</mj-text>`,
      `<mj-text padding="0" align="right">${airportInnerHtml("Destino", destination.name, destination.code)}</mj-text>`,
      30,
      40,
      30,
      `padding-bottom="16px"`
    );
    return `${labelSection}${row}`;
  }
  ...
  const dateSection = section1(
    `<mj-text padding="0"><div style="font-size:12px;color:${COLOR_TEXT_SECONDARY};">${dateLabel}</div></mj-text>`,
    `padding-bottom="6px"`
  );
  const row = section3(
    `<mj-text padding="0" align="left">${airportInnerHtml("Salida", formatTimeLabel(first.departure), first.origin)}</mj-text>`,
    `<mj-text padding="0" align="center">${connectorInnerHtml(centerLabel)}</mj-text>`,
    `<mj-text padding="0" align="right">${airportInnerHtml("Llegada", formatTimeLabel(last.arrival), last.destination)}</mj-text>`,
    30,
    40,
    30,
    `padding-bottom="16px"`
  );
  return `${labelSection}${dateSection}${row}`;
}
...
function buildTripLayoutMjml(best, isRoundtrip, schedule, origin, destination) {
  ...
  const divider = section1(`<mj-divider border-width="1px" border-style="dashed" border-color="#eeeeee" padding="0" />`, `padding="4px 0 12px 0"`);
...
// Shared card shell (accent top border, header row with route+trip tag on
// the left and airline badge on the right, trip layout, an optional note
// row, an optional extra row, then price + CTA button) used by the summary
// route cards, the Google Flights section and the alert cards -- the three
// previously had this structure copy-pasted three times.
function buildTripCardMjml({ ruta, isRoundtrip, airlineHtml, legsMjml, noteHtml, extraMjml, priceBlockHtml, ctaLink, ctaLabel, accentColor }) {
  const headerRow = section2(
    `<mj-text padding="0"><div style="font-size:16px;color:${COLOR_TEXT};font-weight:bold;">📍 ${ruta}</div><div style="margin-top:8px;">${buildTripTypeTag(isRoundtrip)}</div></mj-text>`,
    `<mj-text padding="0" align="right"><div style="font-size:12px;font-weight:600;color:${COLOR_TEXT_SECONDARY};text-transform:uppercase;letter-spacing:0.5px;margin-bottom:4px;">Aerolínea</div>${airlineHtml}</mj-text>`,
    65,
    35,
    `padding-bottom="16px"`
  );
  const noteRow = noteHtml ? section1(`<mj-text padding="0">${noteHtml}</mj-text>`, `padding-bottom="8px"`) : "";
  const ctaColumn = ctaLink
    ? `<mj-column width="35%" padding="0"><mj-button href="${ctaLink}" background-color="${COLOR_PRIMARY}" color="#ffffff" font-size="14px" font-weight="600" border-radius="${RADIUS}" padding="0" inner-padding="10px 18px" align="right">${ctaLabel}</mj-button></mj-column>`
    : mjColumn(35, "");
  const footerRow = mjSection(
    mjColumn(65, `<mj-text padding="0">${priceBlockHtml}</mj-text>`) + ctaColumn,
    `border-top="1px solid ${COLOR_BORDER}" padding-top="16px"`
  );

  return `<mj-wrapper border="1px solid ${COLOR_BORDER}" border-top="4px solid ${accentColor}" border-radius="${RADIUS}" padding="16px" background-color="${COLOR_SURFACE}">
    ${headerRow}
    ${legsMjml}
    ${noteRow}
    ${extraMjml || ""}
...
function buildRouteCards(rows, brlToArsRate, historyStats, routeIdLookup) {
  ...
  for (const [groupKey, routeRows] of byRoute) {
    ...
    mjml += buildTripCardMjml({
      ruta,
      isRoundtrip,
      airlineHtml: buildAirlineLogoHtml(best.mejor_sitio),
...
function buildDashboardRouteBoxMjml(route) {
  ...
  const content = route.link
    ...
    : inner;
  return `<mj-column width="50%" padding="4px" border="1px solid ${COLOR_BORDER}" border-radius="${RADIUS}" background-color="${COLOR_SURFACE}"><mj-text padding="12px">${content}</mj-text></mj-column>`;
}
...
function buildDashboardHtml(summary) {
  ...
  return `<mj-wrapper background-color="${COLOR_BG}" border-radius="${RADIUS}" padding="16px">
    ${boxesMjml}
...
// just a routine update with nothing actionable. Always greets the recipient
// by name so it's clear at a glance in the inbox who this run is for.
function buildSubject(summary, prefix, name) {
  const greeting = `Buenos días${name ? " " + name : ""}: acá está tu resumen diario ✈️`;
  if (summary.newMinCount > 0) return `${prefix}🔥 ${greeting} (nuevo mejor precio)`;
  if (summary.droppedCount > 0) return `${prefix}✈️ ${greeting} (bajaron precios)`;
  return `${prefix}${greeting}`;
}

const SITE_URL = "https://pablonicolasacevedo.github.io/vuelos-tracker/";

// Builds the full MJML document (page background, white rounded card, title,
// dashboard, body, footer) and compiles it to HTML. Using MJML instead of a
// hand-written <table>-based shell means the viewport meta tag, per-client
// media queries and mobile column-stacking all come from the compiler
// instead of being maintained by hand -- that's what was causing the
// responsiveness/styling problems on narrow screens.
...
async function wrapEmail(title, dashboardMjml, bodyMjml, { attachmentNote = true } = {}) {
  // Title/attachment-note/link/footer are plain text blocks (no card
  // background needed), so they're top-level sections rather than
  // mj-wrapper -- MJML doesn't allow nesting mj-wrapper inside mj-wrapper,
  // and the dashboard/route/alert cards below are each their own wrapper.
  const titleSection = section1(
    `<mj-text padding="0"><h2 style="margin:0 0 16px;font-size:32px;font-weight:800;color:${COLOR_TEXT};">✈️ ${title}</h2></mj-text>`,
  ...
  const footerSection = section1(
    `<mj-text padding="0"><p style="font-size:14px;color:${COLOR_TEXT_SECONDARY};margin-top:16px;">Generado automáticamente por vuelos-tracker.</p></mj-text>`,
  ...
  <mj-head>
    ...
    <mj-font name="Plus Jakarta Sans" href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap" />
  </mj-head>
...
function buildAlertCardHtml(alert, brlToArsRate) {
  ...
  return buildTripCardMjml({
    ruta,
    isRoundtrip,
    airlineHtml: buildAirlineLogoHtml(site),
...
function buildAnnouncementHtml() {
  return `<mj-wrapper background-color="#eef2ff" border="1px solid ${COLOR_BORDER}" border-radius="${RADIUS}" padding="16px">
...
function buildOnboardingHtml(name) {
  return `<mj-wrapper background-color="#eef2ff" border="1px solid ${COLOR_BORDER}" border-radius="${RADIUS}" padding="16px">
    ${section1(`<mj-text padding="0">
      ...
      <p style="font-size:14px;color:${COLOR_TEXT_SECONDARY};margin:0 0 8px;line-height:1.6;">Este es tu tracker de precios de vuelos. Así funciona:</p>
      <ul style="margin:0;padding-left:20px;font-size:14px;color:${COLOR_TEXT_SECONDARY};line-height:1.6;">
...
async function sendWelcomeEmail(person) {
  ...
  await transporter.sendMail({
    from: process.env.GMAIL_USER,
    to,
    subject: `¡Bienvenida, ${name}! 👋 Así funciona tu tracker de vuelos`,
