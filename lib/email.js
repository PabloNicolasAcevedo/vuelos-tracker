...
function buildAirlineLogoHtml(site) {
  ...
  return `<span style="font-size:14px;font-weight:700;color:${info.color};letter-spacing:0.3px;">${info.name}</span>`;
}
...
function connectorCellHtml(centerLabel) {
  return `<td width="40%" style="vertical-align:middle;text-align:center;padding:0 8px;">
    ...
    <div style="border-top:1px solid #ccc;line-height:0;">
      <span style="background:${COLOR_SURFACE};padding:0 6px;font-size:14px;color:${COLOR_TEXT_SECONDARY};position:relative;top:-9px;font-family:'Segoe UI Symbol',Arial,sans-serif;">&#9992;&#xFE0E;</span>
    </div>
  </td>`;
}
...
function buildRouteCards(rows, brlToArsRate, historyStats, routeIdLookup) {
  ...
  for (const [groupKey, routeRows] of byRoute) {
    ...
    summary.routes.push({
      ...
      avgFormatted: stats && stats.avg ? formatMoney(stats.avg, stats.currency) : null,
    });
...
function buildDashboardRouteBoxHtml(route) {
  const avgLine = route.avgFormatted ? ` · prom. ${route.avgFormatted}` : "";
  const trendHtml =
    route.vsAvgPercent == null
      ? `<div style="font-size:13px;color:${COLOR_TEXT_SECONDARY};">— vs promedio</div>`
      : `<div style="font-size:13px;color:${route.vsAvgPercent <= 0 ? COLOR_SUCCESS : COLOR_DANGER};">${route.vsAvgPercent <= 0 ? "↘" : "↗"} ${route.vsAvgPercent > 0 ? "+" : ""}${route.vsAvgPercent}% vs promedio${avgLine}</div>`;
  const info = airlineInfoFor(route.site);
  const airlineBadge = `<span style="font-size:11px;font-weight:700;color:${info.color};">${info.name}</span>`;
  ...
  const inner = `
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
      ...
      <tr>
        ...
        <td style="text-align:right;vertical-align:bottom;">${airlineBadge}</td>
      </tr>
      <tr>
...
function buildGoogleFlightsSectionHtml(historyStats, brlToArsRate) {
  ...
  for (const route of googleFlightsRoutes) {
    ...
    const logoHtml = `<span style="font-size:14px;font-weight:700;color:${airlineColor};letter-spacing:0.3px;">${airlineName}</span>`;
