...
<html>
  ...
  <body style="background:${COLOR_BG};margin:0;padding:16px 0;">
    <div style="max-width:600px;margin:0 auto;font-family:${FONT_STACK};color:${COLOR_TEXT};background:${COLOR_SURFACE};padding:24px;border-radius:${RADIUS};">
      ...
      <p style="font-size:14px;color:${COLOR_TEXT_SECONDARY};margin-top:16px;">🔗 Todas las rutas y fechas, con gráficos de evolución de precio: <a href="${SITE_URL}" style="color:${COLOR_PRIMARY};">${SITE_URL}</a></p>
...
async function sendWelcomeEmail(person) {
  ...
  await transporter.sendMail({
    ...
    html: wrapEmail(
      config.title,
      buildDashboardHtml(result.summary),
      buildOnboardingHtml(name) + result.html
    ),
