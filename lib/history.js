...
const { listPriceFiles } = require("./store");
...
function loadRawPriceRows() {
  const rows = [];
  for (const filePath of listPriceFiles()) {
    // git's core.autocrlf can check these files out with \r\n while our own
    ...
    // confused on this data (many rows end in an empty trailing field) and
    // merges dozens of rows into one, throwing CSV_RECORD_INCONSISTENT_COLUMNS.
    rows.push(...parse(content, { columns: true, skip_empty_lines: true, record_delimiter: "\n" }));
  }
  return rows;
}
