...
// Google Flights aggregates whichever airline is actually cheapest (LATAM,
// Gol, etc.) -- surface that real operating airline instead of the generic
// "Google Flights" bucket, otherwise routes where LATAM wins never show it
// by name even though it's the flight we actually captured.
function airlineFor(site, latestNotes) {
  if (site !== "googleflights") return null;
  try {
    return JSON.parse(latestNotes || "{}").out?.segments?.[0]?.airline || null;
  } catch {
    return null;
  }
}
  ...
  for (const route of routes) {
    ...
    const routeRows = rows.filter(
      (row) =>
        ...
        (!route.toDate || row.depart_date <= route.toDate)
    );
      ...
      .map(({ row, sites }) => {
        ...
        for (const site of [...sites].sort()) {
          ...
          option.current[site] = {
            ...
            airline: airlineFor(site, st.latestNotes),
          };
          option.history[site] = {
            currency: st.currency,
            min: st.min,
